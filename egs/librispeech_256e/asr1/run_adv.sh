...
resume="exp/train_600_pytorch_train_specaug/results/snapshot.ep.13"        # Resume the training from snapshot
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    #for part in dev-clean test-clean dev-other test-other train-clean-100 train-clean-360; do
    for part in dev-clean test-clean dev-other test-other train-clean-100 train-other-500; do
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for x in dev_clean test_clean dev_other test_other train_clean_100 train_other_500; do
    ...
    utils/combine_data.sh --extra_files utt2num_frames data/${train_set}_org data/train_clean_100 data/train_other_500
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    # Make speaker ids for train_600 (data/spk2id)
    local/make_spkid.sh ${asr_data} "train-clean-100|train-other-500"
