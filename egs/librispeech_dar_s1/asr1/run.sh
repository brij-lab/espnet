...
lang_model=rnnlm.model.bestmod # set a language model to be used for decoding
...
# exp tag
tag="_dar_s2_16k" # tag for managing experiments.
...
if [ ${stage} -le -1 ] && [ ${stop_stage} -ge -1 ]; then
    echo "stage -1: Syncing transcripts"
    rsync -avzm --ignore-existing $datadir/LibriSpeech/train-clean-360/* $datadir/LibriSpeech${tag}/train-clean-360/
    rsync -avzm --ignore-existing $datadir/LibriSpeech/train-clean-100/* $datadir/LibriSpeech${tag}/train-clean-100/
fi
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    for part in train-clean-100 train-clean-360; do
        ...
        local/data_prep.sh ${datadir}/LibriSpeech${tag}/${part} data/${part//-/_}
    done
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    #for x in train_clean_100 train_clean_360; do
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    recog_set_arr=("test_clean" "test_other" "dev_clean" "dev_other")
    #for rtask in ${recog_set}; do
    for ((i=0; i<${#recog_set_arr[*]}; i++)); do
    (
    	rtask=${recog_set_arr[i]}
    	echo "Decoding : " $rtask
